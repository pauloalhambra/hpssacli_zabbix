...
echo -n ']}'
[root@SBC-2-TG scripts]# ls
check_raid_ld_status.sh  hp-raid-ctrl-discovery.sh  hp-raid-data-processor.sh  hp-raid-ld-discovery.sh  hp-raid-pd-discovery.sh  hwraid-hpacucli
[root@SBC-2-TG scripts]# cat hp-raid-data-processor.sh
#!/bin/bash
...
for slot in $ctrl_list; 
  do
    ld_list=$($hpssacli ctrl slot=$slot ld all show |grep -w logicaldrive |awk '{print $2}' |xargs echo)
  for ld in $ld_list;
    do
...
for slot in $ctrl_list; 
  do
    pd_list=$($hpssacli ctrl slot=$slot pd all show |grep -w physicaldrive |awk '{print $2}' |xargs echo)
  for pd in $pd_list;
    do
...
for c in $(/etc/zabbix/scripts/hwraid-hpacucli/hp-raid-ctrl-discovery.sh raw); 
  do
...
for l in $(/etc/zabbix/scripts/hwraid-hpacucli/hp-raid-ld-discovery.sh raw);
  do
    echo -n -e "hpraid.ld.status[$l]\n";
  done >> $all_keys

for p in $(/etc/zabbix/scripts/hwraid-hpacucli/hp-raid-pd-discovery.sh raw);
  do
